...
# 忽略大小写去匹配字符
print(re.findall(r'[a-z]+',s1,re.I))

# 匹配以a开头c结尾的字符串
print(re.findall(r'a.*c',s1))


"""
    re.search(): 只会匹配符合规则的第一个值  不要求开头必须匹配 只要字符串中符合条件即会返回  Match对象
"""
...
"""
    实际场景用的很少
    re.match():  尝试使用正则表达式从字符串的起始位置开始匹配，只有开头就符合才返回匹配结果.
    否则返回None  验证是否符合特定开头格式
    
    re.fullmatch(): 严格要求整个字符串都匹配
"""
text = "zworld 123 hello world"
# res = re.match("world",text)
res = re.fullmatch("z",text)
print(res)

"""
    re.sub():  将匹配的内容替换为其他内容
    re.subn():  将匹配的内容替换为其他内容 同时返回替换次数
...
"""
    re.split : 将正则表达式 匹配到的内容变成分割符进行拆分字符串
"""
s = "xyg ,*&* time *^* dog cat"
print(re.split(r'\W+', s))

"""
    re.complie: 预编译正则表达式  将正则表达式内容转化为正则表达式对象
"""
